<template>
  <div class="dash_sub_wrap">
    ...
    <overlay :close.sync="close1">
      <touyun :type='9' :ids="operation.join(',')" @tiuyun="operate(operation)" @cancel="close1=false"></touyun>
    </overlay>
    <overlay :close.sync="close2">
      <tuiyi :type='9' :ids="operation.join(',')" @tuiyi="reoperate(operation)" @cancel="close2=false"></tuiyi>
    </overlay>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    saveEquipment() {
      ...
      this.$http
        .post(
          "/maintenanceService/maintenance/updateBreaker?sign=" +
  ...
  mounted() {
    this.$nextTick(() => {
      let scroll6 = new BScroll(".dash_sub1_table", {
        scrollbar: true,
        mouseWheel: true,
