<template lang="html">
  <div class="dash_sub_wrap">
        ...
        <table class="table">
          <tbody>
            <tr v-for="(item,index) in dataList">
              ...
              <td width="20%" @click="close=true;getItem(item)"><span class="color_warning">{{item.mrid}}</span></td>
    ...
    <overlay :close.sync="close1">
      <touyun :type='4' :ids="operation.join(',')" @tiuyun="operate(operation)" @cancel="close1=false"></touyun>
    </overlay>
    <overlay :close.sync="close2">
      <tuiyi :type='4' :ids="operation.join(',')" @tuiyi="reoperate(operation)" @cancel="close2=false"></tuiyi>
    </overlay>
  </div>
</template>
<script>
...
export default {
  ...
  created() {
    this.callbackdata.list.forEach(item => {
      item.lineEndModels.forEach(arr => {
  ...
  methods: {
    saveEquipment() {
      ...
      this.$http
        .post(
          "/maintenanceService/maintenance/updateAclineend?sign=" +
  ...
  mounted() {
    this.$nextTick(() => {
      let scroll5 = new BScroll(".dash_sub1_table", {
        scrollbar: true,
        mouseWheel: true,
  ...
  watch: {
    operation: function(news) {
      if (this.operation.length == this.dataList.length) {
