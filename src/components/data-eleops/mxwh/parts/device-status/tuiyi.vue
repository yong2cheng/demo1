              ...
              <div class="tc_count_tool clearfix">
                <ul class="clearfix">
                    ...
                    <li style="display: inline-block;position: relative;margin-right: 15px;">
                        ...
                        <div v-show="isFocus" style="position:absolute;background:#012323;height:200px;width:172px;border-radius:5px;overflow:hidden;z-index:100;left: 66px;" class="subStationScroll3">
                            <ul>
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table">
                        <thead>
                            <tr>
                                ...
                                <td width="8%" style="position: relative;">记录创建人
                                  <div class="common_sort">
                                ...
                                <td>原因</td>
                                <td width="8%" style="position: relative;">传送状态
                                  <div class="common_sort">
                    ...
                    <table class="table">
                        <tbody>
                            <tr v-for="(item,index) in callbackdata" :key="index">
                               ...
                               <td width="8%">{{item.creatUser}}</td>
                               <td>
                                <span class="reasonWhy" :title="item.remark">{{item.remark}}</span>
                               </td>
                               <td width="8%">{{item.sgztName}}</td>
          ...
          <overlay :close.sync="showDetail">
              <div style="width: 700px;height: 520px;margin: 0 10px">
                <div class="tc_count_title">
                    <h2 class="clearfix">退役计量点详情</h2>
                </div>
                <div class="tc_count_body">
                    <div class="getdetail_scroll3" style="position: relative;height: 400px;overflow: hidden;">
                        <table class="table">
                          <tbody>
                            ...
                            <tr><td width="25%">退役时间</td><td>{{sbDetail.retireTime}}</td></tr>
...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then(res => {
          if (res.data.code === 1) {
            ...
            this.$nextTick(() => {
                this.status_scroll2 = new BScroll('.status_scroll1', {scrollbar:{fade: false,interactive: true} ,mouseWheel: true,preventDefault:false});
            })
            this.loading =false
          }
        });
    ...
    clickInput() {
      this.isFocus = true;
      if(this.subStationScroll3) {
        this.subStationScroll3.destroy()
      }
      this.$nextTick(() => {
        this.subStationScroll3 = new BScroll(".subStationScroll3", {scrollbar:{fade: false,interactive: true} ,mouseWheel: true,preventDefault:false});
      });
    },
    clickDetail() {
      if(this.getdetail_scroll3) {
        this.getdetail_scroll3.destroy()
      }
      this.$nextTick(() => {
        this.getdetail_scroll3 = new BScroll(".getdetail_scroll3", {scrollbar:{fade: false,interactive: true} ,mouseWheel: true,preventDefault:false});
      });
    }
  },
};
...
.subStationScroll3 ul li:hover {color: #fff}
</style>
